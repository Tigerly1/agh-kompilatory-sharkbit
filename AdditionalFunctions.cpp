...
public:
	...
	void search(const HANDLE process, const int passed_val) {
		...
		unsigned char* buffer = (unsigned char*)calloc(1, size_block);
		size_t bytes_read = 0;

		for (DWORD i = 0x00000000; i < 0x7FFFFFFF; i += size_block) {
			ReadProcessMemory(process, (void*)i, buffer, size_block, &bytes_read);

			for (int j = 0; j < size_block - 4; j += 4) {
				DWORD val = 0;
	...
	int run(int valueAtStart, int valueAtEnd, int ValueAdded) {
		...
		do {
			...
			if (strcmp(pe32.szExeFile, "wesnoth.exe") == 0) {
				...
				search(process, valueAtStart);
				std::cout << "You have 20 seconds to make changes!!";
				Sleep(20000);
				filter(process, valueAtEnd);
				write(process, ValueAdded);
